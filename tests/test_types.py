...
from retrievers.context.base import Context
from retrievers.exceptions import SchemaError
from retrievers.types import (
    Array,
    InnerField,
    Schema,
    SchemaField,
...
def test_context_schemas():
    ...
    assert (
        email_schema["subject"].type == "string" and email_schema["subject"].size == 100
    )
    assert (
        email_schema["recipients"].type == "array"
        and email_schema["recipients"].inner_type.type == "string"
    )
    ...
    assert (
        employee_schema["id"].is_id is True and employee_schema["id"].type == "string"
    )
    assert (
        employee_schema["name"].type == "string" and employee_schema["name"].size == 50
    )
    assert employee_schema["age"].type == "integer"
    assert (
        employee_schema["salary"].type == "number"
        and employee_schema["salary"].format == "double"
    )
    assert employee_schema["active"].type == "boolean"
    assert (
        employee_schema["skills"].type == "array"
        and employee_schema["skills"].inner_type.type == "string"
    )

    company_schema = Company.schema().as_dict()
    assert company_schema["id"].is_unique is True
    assert (
        company_schema["name"].type == "string" and company_schema["name"].size == 100
    )
...
def test_schema_field_and_constraints():
    ...
    s = Schema(
        {
            ...
            "name": SchemaField(
                type="string", format=None, size=10, optional=False, inner_type=None
            ),
        }
    )
    d = s.as_dict()
    ...
    with pytest.raises(SchemaError):
        Schema(
            {
                "arr": SchemaField(
                    type="array", format=None, size=3, optional=False, inner_type=None
                )
            }
        )
    ...
    assert (
        SchemaField(
            type="number", format=None, optional=False, size=None, inner_type=None
        ).format
        == "float"
    )
    ...
    minimal = Schema(
        {
            ...
            "title": SchemaField(
                type="string", format=None, size=32, optional=False, inner_type=None
            ),
        }
    )
    d = minimal.as_dict()
    assert (
        d["id"].is_id is True and d["title"].type == "string" and d["title"].size == 32
    )
