...
from langchain_community.query_constructors.milvus import (
    MilvusTranslator as MilvusTranslator_,
)
...
class MilvusBackend:
            ...
            for field_name, field_value in record.items():
                if field_value is None:
                    ...
                    if schema[field_name].type == "string":
                        ...
                        or schema[field_name].type == "integer"
                    ):
                        record[field_name] = 0
    ...
    def create_collection(
        ...
        if not issubclass(payload_class, Context):
            raise TypeError(
                f"Payload class {payload_class} is must be a subclass of Context"
            )

        schema = _modaic_to_milvus_schema(
            self._client, payload_class.schema(), self.milvus_lite
        )
        ...
        try:
            ...
            raise ValueError(
                f"Milvus does not support vector type: {index.vector_type}"
            ) from None
        kwargs = {
        ...
        try:
            ...
            raise ValueError(
                f"Milvus does not support metric type: {index.metric}"
            ) from None
                    ...
                    case {"id": id, "distance": distance, "entity": entity}:
                        context_list.append(
                            SearchResult(
                                id=id,
                                score=distance,
                                context=payload_class.model_validate(
                                    self._process_null(entity)
                                ),
                            )
                        )
                    case _:
                        raise ValueError(
                            f"Failed to parse search results to {payload_class.__name__}: {result}"
                        )
    ...
    def get_records(
        ...
        records = self._client.get(
            collection_name=collection_name, ids=record_ids, output_fields=output_fields
        )
        return [
            payload_class.model_validate(self._process_null(record))
...
def _modaic_to_milvus_schema(
    client: MilvusClient, modaic_schema: Schema, milvus_lite: bool
) -> CollectionSchema:
    """
    ...
    Args:
        client: The Milvus client instance
        modaic_schema: The Retrievers schema to convert
