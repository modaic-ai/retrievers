...
from ...query_language import Condition, parse_retrievers_filter
...
class VectorDatabase(Generic[TBackend], Trackable):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if isinstance(payload_class, type) and not issubclass(payload_class, Context):
            raise TypeError(
                f"payload_class must be a subclass of Context, got {payload_class}"
            )
    ...
    def _embed_and_add_records(
        ...
        collection_name: str,
        embedmes: Dict[str, List[str | Image.Image]]
        | Dict[None, List[str | Image.Image]],
        ...
        try:
            ...
            raise ValueError(
                f"Failed to create embeddings for index: {index_name}"
            ) from e
    ...
    def search(
        ...
        if filter is not None:
            filter = parse_retrievers_filter(self.ext.backend.mql_translator, filter)
    ...
    def get_records(self, collection_name: str, record_id: List[str]) -> List[Context]:
        ...
        """
        return self.ext.backend.get_records(
            collection_name, self.collections[collection_name].payload_class, record_id
        )
    ...
    def upsert_records(self, collection_name: str, records: Iterable[Context]):
        ...
        """
        raise NotImplementedError(
            "upsert_record is not implemented for this vector database"
        )
    ...
    def delete_records(self, collection_name: str, context_ids: Iterable[str]):
        ...
        """
        raise NotImplementedError(
            "delete_record is not implemented for this vector database"
        )
