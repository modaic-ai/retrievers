...
if TYPE_CHECKING:
    from retrievers.databases.vector_database.vector_database import VectorDatabase
MAX_IN_FLIGHT = 8
...
def add_records(
    ...
    def gen_embeded_records():
        ...
        for item in records:
            ...
            if batch_size is not None and len(serialized_contexts) == batch_size:
                yield _embed_and_create_records(
    ...
    with ThreadPoolExecutor(max_workers=8) as pool:
        ...
        for records in gen_embeded_records():
            ...
            pending.add(
                pool.submit(self.ext.backend.add_records, collection_name, records)
            )
