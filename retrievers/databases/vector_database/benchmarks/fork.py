...
if TYPE_CHECKING:
    from retrievers.databases.vector_database.vector_database import (
        VectorDatabase,
        VectorDBBackend,
    )
MAX_IN_FLIGHT = 8
...
def add_records(
    ...
    def gen_embeded_records():
        ...
        for item in records:
            ...
            if batch_size is not None and len(serialized_contexts) == batch_size:
                yield (
                    _embed_records(self, collection_name, embedmes),
                    serialized_contexts,
                )
                if embedme_scope == "index":
    ...
    with ProcessPoolExecutor(max_workers=4) as pool:
        ...
        for records in gen_embeded_records():
            ...
            pending.add(
                pool.submit(
                    backend_add_records, self.ext.backend, collection_name, records
                )
            )
