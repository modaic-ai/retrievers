...
from retrievers.context.base import Context, Embeddable

from .common import _add_item_embedme, _items_have_multiple_embedmes

if TYPE_CHECKING:
    from retrievers.databases.vector_database import VectorDatabase
...
def _embed_and_add_records(
    ...
    try:
        ...
        for index_name, index_config in vdb.collections[
            collection_name
        ].indexes.items():
...
def add_records(
    ...
    if embedme_scope == "index":
        embedmes: Dict[str, List[str | Image.Image]] = {
            k: [] for k in vdb.collections[collection_name].indexes.keys()
        }
    else:
        ...
        for _, item in tqdm(
            ...
            if batch_size is not None and len(serialized_contexts) == batch_size:
                _embed_and_add_records(
                    vdb, collection_name, embedmes, serialized_contexts
                )
                if embedme_scope == "index":
