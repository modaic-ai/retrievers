...
if t.TYPE_CHECKING:
    ...
    from retrievers.databases.graph_database import GraphDatabase
    from retrievers.storage.file_store import FileStore
    ...
    def wrapper(*args, **kwargs):
        ...
        for attr in self.__class__.__hydrated_attributes__:
            if getattr(self, attr) is None:
                raise RetrieversHydrationError(
...
def _get_unhidden_serializer(cls: type[BaseModel]) -> SchemaSerializer:
    ...
    def walk(node: dict | list):
        if isinstance(node, dict):
            if (
                node.get("type") == "model"
                and node.get("serialization", {}).get("function", None)
                is Context.hidden_serializer
            ):
...
class Context(BaseModel, metaclass=ContextMeta):
    ...
    _gqlalchemy_class_registry: t.ClassVar[
        t.Dict[str, t.Type["gqlalchemy.models.GraphObject"]]
    ] = {}
    ...
    def to_gqlalchemy(self, db: "GraphDatabase") -> "gqlalchemy.Node":
        ...
        try:
            ...
            raise ImportError(
                "GQLAlchemy is not installed. Please install the graph extension for retrievers with `uv add retrievers[graph]`"
            ) from None
        assert isinstance(db, GraphDatabase), (
            f"Expected db to be a retrievers.databases.GraphDatabase instance. Got {type(db)} instead."
        )
        cls = self.__class__
    ...
    def _best_subclass(labels: t.FrozenSet[str]) -> t.Type["Context"]:
        ...
        if best_subclass is None:
            raise ValueError(
                f"Cannot find a matching Context class for labels: {labels}"
            )
    ...
    def is_hydrated(self) -> bool:
        ...
        if not hasattr(self, "__hydrated_attributes__"):
            return True
        return all(
            getattr(self, attr) is not None for attr in self.__hydrated_attributes__
        )
...
class Relation(Context, metaclass=RelationMeta):
    ...
    @t.overload
    def __init__(
        self, start_node: Context | int, end_node: Context | int, **data: Any
    ) -> "Relation": ...
    ...
    def get_end_node_obj(self, db: "GraphDatabase") -> Context:
        ...
        if self._end_node:
            ...
            return Context.from_gqlalchemy(
                next(
                    db.execute_and_fetch(
                        f"MATCH (n) WHERE id(n) = {self.end_node} RETURN n"
                    )
                )
            )
    ...
    def __str__(self):
        ...
        """
        fields_repr = ", ".join(
            f"{k}={repr(v)}" for k, v in self.model_dump(include_hidden=True).items()
        )
    ...
    def save(self, db: "GraphDatabase"):
        ...
        for k in self.model_dump(
            exclude={"id", "start_node", "end_node"}, include_hidden=True
        ):
            setattr(self, k, getattr(result, k))
        self._gqlalchemy_id = result._id
...
def _cast_type_if_base_model(field_type: t.Type) -> t.Type:
    ...
    if origin is dict:
        ...
        return dict[
            _cast_type_if_base_model(key_type), _cast_type_if_base_model(value_type)
        ]
    if origin is tuple:
        ...
        return tuple[
            tuple([_cast_type_if_base_model(a) for a in args])
        ]  # tuple[(A, B, C)]
...
def get_annotations(
    cls: t.Type, exclude: t.Optional[t.List[str]] = None
) -> t.Dict[str, t.Type]:
    if exclude is None:
        exclude = []
...
def get_defaults(
    cls: t.Type[Context], exclude: t.Optional[t.List[str]] = None
) -> t.Dict[str, t.Any]:
    if exclude is None:
        exclude = []
...
class Hydratable(t.Protocol):
    ...
    @classmethod
    def from_file(
        cls, file: str, file_store: FileStore, type: str, params: dict = None
    ) -> "Hydratable":
        """
