...
class Condition:
    ...
    def __str__(self):
        return f"Prop({self.condition.attribute}) {comparator_to_python[self.condition.comparator]} {self.condition.value}"

    def __contains__(self, other: str):
        raise ValueError(
            "Retrievers Filters do not support `in` use Prop.in_()/Prop.not_in() instead"
        )

    def __bool__(self):
        raise ValueError(
            "Attempted to evaluate Retrievers condition as boolean. Please make sure you wrap ALL expresions with ()"
        )
    ...
    def __invert__(self):
        ...
        raise NotImplementedError("Retrievers queries do not support ! yet")
...
def _enforce_types(
    ...
    if other_type not in allowed_types:
        raise ValueError(
            f"Value must be one of {allowed_types}, got {other_type} for {op}"
        )
...
def _get_type(value: ValueType):
    if isinstance(value, Value):
        ...
        raise NotImplementedError(
            "Retrievers queries do not support comparisions between properties yet."
        )
    else:
        raise ValueError(
            f"Unssuported value type in Retrievers condition, got {type(value)}"
        )
...
class Prop:
    ...
    def __getitem__(self, key: str):
        raise NotImplementedError(
            "Retrievers queries do not support nested properties yet"
        )
    ...
    def comparison(
        self, op_expr: str, comparator: Comparator, other: "Value"
    ) -> "Condition":
...
def parse_retrievers_filter(translator: Visitor, condition: Condition) -> dict:  # noqa: N802
