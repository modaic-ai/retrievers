...
class SchemaField(BaseModel):
    ...
    def from_json_schema_property(
        ...
        if "maxItems" in inspected_type and "maxLength" in inspected_type:
            raise SchemaError(
                "maxItems and maxLength cannot both be present in a schema field"
            )
        ...
        return SchemaField(
            ...
            size=inspected_type.get("maxItems", None)
            or inspected_type.get("maxLength", None),
...
class InnerField(BaseModel):
    ...
    def from_json_schema_property(inner_schema: dict) -> "InnerField":
        ...
        if inspected_type["type"] == "object" or inspected_type["type"] == "array":
            raise SchemaError(
                "Arrays and Dicts are not supported for Array/List elements"
            )
        return InnerField(
            type=inspected_type["type"],
            format=inspected_type.get("format", None),
...
class Schema(RootModel[dict[str, SchemaField]]):
    ...
    def validate_is_id(cls, v: dict[str, SchemaField]) -> dict[str, SchemaField]:
        ...
        if offenders:
            raise SchemaError(
                "is_id can only be True for the key 'id'; offending keys: "
    ...
    def from_json_schema(schema: dict) -> "Schema":
        ...
        for field_name, field_schema in schema["properties"].items():
            schema_field = SchemaField.from_json_schema_property(
                field_schema, is_id=field_name == "id"
            )
...
def _inspect_type(field_schema: dict) -> Tuple[dict, bool]:
    """
    This function inspects the json schema ensuring it is a valid retrievers schema. Returns from this function are guaranteed to:
    ...
    if anyOf := field_schema.get("anyOf", None):  # noqa: N806
        if len(anyOf) > 2 or not any(_is_null(item) for item in anyOf):
            raise SchemaError("Unions are not supported for Retrievers Schemas")
...
def Field(
    default: Any = PydanticUndefined, *, hidden: bool = False, **kwargs
) -> FieldInfo:  # noqa: N802, ANN001
    if hidden:
        return PydanticField(
            default=default, **kwargs, json_schema_extra={"hidden": True}
        )
    else:
