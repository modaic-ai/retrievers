...
```python
from retrievers.context import TableFile
...
```python
from retrievers.context import Hydratable, Text, TableFile
isinstance(Text(text="hi"), Hydratable)      # False
isinstance(table, Hydratable)                 # True
```

<Warning>`Hydratable` only checks method presence, not that the object is a `Context`. Use `retrievers.context.is_hydratable(obj)` to check both.</Warning>
...
```python
from retrievers.context import Embeddable, MultiEmbeddable
...
Manager() → labels ['Employee', 'Manager']
```
</Tip>
...
```python
from retrievers.context import Relation, Context
