...
Retrievers provides a small, ergonomic ORM over [GQLAlchemy](https://memgraph.github.io/gqlalchemy/) to work with graph databases (Neo4j, Memgraph) using `Context` and `Relation` classes. This guide mirrors the style of our SQL guide and focuses on practical, copy‑pasteable examples.
...
### MemgraphConfig
```python
from retrievers.databases import GraphDatabase, MemgraphConfig
...
## See also
- `retrievers.context.base.Context` and `Relation` for the ORM model
- Reference pages for `retrievers.databases.graph_database` configs